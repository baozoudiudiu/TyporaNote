...
* 定义`类名`，`结构体名`，`枚举名`，`文件名`等防止和第三方命名重复，需要添加特定的前缀，以`Ripple`开发项目为例，所有命名的地方会添加`RP`前缀。以自定义一个UIView为例子：

  ```swift
  /// swift
  class RPCustomView: UIView {
  	// your code
  }
  ```

* `文件夹命名`和`文件命名`遵循驼峰规则，以大写字母开头。

* `变量名`和`方法名`遵循驼峰规则，以小写字母开头。

* 整个项目会用到的全局静态变量和方法命名时以`k`+`前缀` 开头，以全局获取屏幕宽高为例：

  ```swift
  /// swift
  let kRPScreenWidth: CGFloat = UIScreen.main.bounds.size.width
  let kRPScreenHeight: CGFloat = UIScreen.main.bounds.size.height
  ```
...
swift中对某个类的extension文件命名格式应遵循`Extension` + `ClassName`的格式。以对系统UIView的extension文件命名为例：`Extension+UIView.swift`。
...
* cocoaPods集成三方库时，版本号请指定特定版本，不要自动指定最新版本。